...
namespace IANodeGraph.View
{
    ...
    public partial class BaseGraphView : GraphView
    {
        ...
        private NodeMenuWindow nodeMenuWindow;
        ...
        public NodeMenuWindow NodeMenuWindow
        {
            get
            {
                if (nodeMenuWindow == null)
                {
                    nodeMenuWindow = ScriptableObject.CreateInstance<NodeMenuWindow>();
                    nodeMenuWindow.Initialize("Nodes", this);
                    BuildNodeMenu(nodeMenuWindow);
                    nodeMenuWindow.QuickSortEntrys();
                }
                return nodeMenuWindow;
            }
        }
        ...
        public void Init()
        {
            ...
            IEnumerator InitCoroutine()
            {
                ...
                viewTransform.position = new Vector3(ViewModel.Pan.Value.x, ViewModel.Pan.Value.y);
                viewTransform.scale = new Vector3(ViewModel.Zoom.Value, ViewModel.Zoom.Value, 1);
        ...
        {
            ViewModel.Pan.RegisterChanged(OnPanChange);
            ViewModel.Zoom.RegisterChanged(OnZoomChange);
        }
        ...
        {
            ViewModel.Pan.UnregisterChanged(OnPanChange);
            ViewModel.Zoom.UnregisterChanged(OnZoomChange);
        }
        ...
        public GroupView AddGroupView(BaseGroupProcessor group)
        {
        ...
        public void ShowNodeMenuWindow(Vector2 pPos)
        {
            SearchWindow.Open(new SearchWindowContext(pPos), NodeMenuWindow);
        }
        ...
        private void OnZoomChange(float pNewZoom, float pOldZoom)
        {
            viewTransform.scale = new Vector3(pNewZoom, pNewZoom, 1);
            SetDirty();
        }

        private void OnPanChange(Vector2Int pNewPan, Vector2Int pOldPan)
        {
            viewTransform.position = new Vector3(pNewPan.x, pNewPan.y);
            SetDirty();
        }
        ...
        private void NodeCreationRequest(NodeCreationContext c)
        {
            ...
            //SearchWindow.Open(new SearchWindowContext(c.screenMousePosition), NodeMenuWindow);
        }
                        ...
                        case BaseNodeView nodeView:
                            {
                                if (nodeView.Movable)
                                {
                                    ...
                                    foreach (var port in nodeView.ViewModel.Ports.Values)
                                    {
                                        ...
                                        {
                                            if (port.Direction == PortDirection.Left)
        ...
        {
            ViewModel.Zoom.Value = viewTransform.scale.x;
            ViewModel.Pan.Value  = new Vector2Int((int)viewTransform.position.x, (int)viewTransform.position.y); 
        }
